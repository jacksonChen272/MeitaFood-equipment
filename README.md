...
### 2026-07-09｜v1.6 QR Code 與欄位建議

- 將原本的一維條碼功能改為設備專屬 QR Code。
- QR Code 內容改為設備資訊連結，格式為 `index.html?asset=設備ID`。
- 掃描 QR Code 開啟頁面後，會自動顯示該設備資訊彈窗。
- QR Code 改為內建產生器，不依賴外部 CDN。
- 支援下載 QR Code PNG。
- 支援列印 QR Code 設備標籤。
- 設備類型由固定下拉改為可自行輸入，也可選擇既有類型。
- 新增/編輯設備時，品牌、型號、部門、使用人、地點、IP、MAC 會提供曾填過資料的建議。
- 設備類型篩選器會自動納入自行新增的類型。
